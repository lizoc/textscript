// -----------------------------------------------------------------------
// <copyright file="ScriptObject.cs" repo="TextScript">
...
namespace Lizoc.TextScript.Runtime
{
    ...
    public class ScriptObject : IDictionary<string, object>, IEnumerable, IScriptObject, IDictionary
    {
        ...
        public ScriptObject(int capacity, bool? autoImportStaticsFromThisType)
        {
            ...
            if (autoImportStaticsFromThisType.GetValueOrDefault() || 
                (!autoImportStaticsFromThisType.HasValue && this.GetType() != typeof(ScriptObject)))
            {
        ...
        bool IDictionary.IsFixedSize
        {
            get
            {
                return ((IDictionary) Store).IsFixedSize;
            }
        }
        ...
        public virtual bool TryGetValue(TemplateContext context, SourceSpan span, string member, out object value)
        {
            ...
            bool result = Store.TryGetValue(member, out internalValue);
        ...
        public virtual string ToString(TemplateContext context, SourceSpan span)
        {
            ...
            foreach (var item in this)
            {
                ...
                KeyValuePair<string, object> keyPair = (KeyValuePair<string, object>)item;
        ...
        {
            ScriptObject toObject = (ScriptObject)MemberwiseClone();
            ...
            if (deep)
            {
                foreach (var keyPair in Store)
                {
                    ...
                    {
                        ScriptObject fromObject = (ScriptObject)value;
                        value = fromObject.Clone(true);
                    }
                    else if (value is ScriptArray)
                    {
                        ScriptArray fromArray = (ScriptArray)value;
        ...
        {
            List<KeyValuePair<string, object>> list = Store.Select(item => new KeyValuePair<string, object>(item.Key, item.Value.Value)).ToList();
        ...
        public static bool IsImportable(object obj)
        {
            ...
            return !(obj is string || typeInfo.IsPrimitive ||
                typeInfo == typeof(decimal).GetTypeInfo() ||
                typeInfo.IsEnum || typeInfo.IsArray);
        }
