// -----------------------------------------------------------------------
// <copyright file="RegexFunctions.cs" repo="TextScript">
...
namespace Lizoc.TextScript.Functions
{
    ...
    public class RegexFunctions : ScriptObject
    {
        ...
        /// </summary>
        /// <param name="context">The template context. This parameter is required to read the timeout configuration.</param>
        ...
        {
#if NETFX || PORTABLE
            Match match = Regex.Match(text, pattern, GetOptions(options));
#else
            Match match = Regex.Match(text, pattern, GetOptions(options), context.RegexTimeOut);
#endif
            ScriptArray matchObject = new ScriptArray();
        ...
        /// - `x`: Eliminates unescaped white space from the pattern and enables comments marked with `#`. 
        /// </param>
        /// <returns>A string array of the splitted text.</returns>
        /// <remarks>
        /// ```template-text
