// -----------------------------------------------------------------------
// <copyright file="TemplateContext.cs" repo="TextScript">
...
namespace Lizoc.TextScript
{
    ...
    public partial class TemplateContext
    {
        ...
        /// <summary>
        /// Gets the current output of the template being rendered (via <see cref="Template.Render(TemplateContext)"/>)/>.
        /// </summary>
        ...
        {
            if (sourceFile == null) throw
                    new ArgumentNullException(nameof(sourceFile));
        ...
        public void PushGlobal(IScriptObject scriptObject)
        {
            if (scriptObject == null)
                new ArgumentNullException(nameof(scriptObject));
        ...
        public void EnterRecursive(ScriptNode node)
        {
            ...
            if (_callDepth > RecursiveLimit)
                throw new ScriptRuntimeException(node.Span, string.Format(RS.RecurseDepthLimitReached, RecursiveLimit, node)); // unit test: 305-func-error2.txt
        }
        ...
        public void ExitRecursive(ScriptNode node)
        {
            _callDepth--;
            if (_callDepth < 0)
                throw new InvalidOperationException(string.Format(RS.ExitRecurseMismatch, node));
        }
        ...
        private object GetOrSetValue(ScriptExpression targetExpression, object valueToSet, bool setter)
        {
            ...
            try
            {
                ...
                {
                    throw new ScriptRuntimeException(targetExpression.Span, string.Format(RS.InvalidExprAssignmentTarget, targetExpression)); // unit test: 105-assign-error1.txt
                }
            }
            catch (Exception readonlyException) when(_getOrSetValueLevel == 1 && !(readonlyException is ScriptRuntimeException))
            {
                throw new ScriptRuntimeException(targetExpression.Span, string.Format(RS.ExprAccessError, targetExpression), readonlyException);
            }
            ...
            bool allowFunctionCall = (_isFunctionCallDisabled && _getOrSetValueLevel > 1) || !_isFunctionCallDisabled;
    ...
    public class LiquidTemplateContext : TemplateContext
    {
            ...
            : base(new LiquidBuiltinsFunctions())
        {
            ...
            TemplateLoaderParserOptions = new ParserOptions() { ConvertLiquidFunctions = true };
        }
    }
}